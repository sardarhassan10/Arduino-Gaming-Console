...
void DinoGame::process(void) {
  ...
  if (button_left.first_pressed()) --refresh_rate;
  //if (button_right.first_pressed()) ++refresh_rate;
  } else {
     if (button_left.first_pressed()) --scroll_speed;
     //if (button_right.first_pressed()) ++scroll_speed;
  }

  if (checkCollision()) display.println("OUCH!");

  score = (time_end - time_start) / 1000;

  if (button_right.first_pressed()) {
    for (int i = 0; i < 5; i++) {
      if (!cactusExists[i]) {
        Cactus cactusObj;
...
bool DinoGame::checkCollision(void) {
  for (unsigned int i = 0; i < 5; i++) {
    ...
    int collision_allowance = 3;

    if ((x + dino_width - collision_allowance >= cactusArray[i].getX()) && (y + dino_height > cactusArray[i].getY())) {
      return true;
    } else {
      return false;
    }
...
void DinoGame::draw(void) {
  if (!game_over) {
    ...
    switch (game_over_state) {
      ...
      display.println(EEPROM.read(EEPROMAdd_temp));
      display.println(EEPROM.read(EEPROMAdd_dino_highscore));
      break;
    }
  }
}
...
void AboutScreen::draw(void) {
  ...
  display.println("NUST SEECS ELECTRICAL ENG.\n\n2018 OOP \nProject");
